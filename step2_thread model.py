...
if STATUS == "ALL":
    ...
    ATTACKfile = "attack_x.csv"
    MODEL = 'final_all.hdf5'
    PREDICTEDy = 'PREDICTION_all.csv'
    PREDICTEDy_csv = 'PREDICTION_all_adv_noRule.csv'
    NOISEx = "adv_all_noRule.csv"
...
df_TRUE,adv = AddNoise(data_x_win,df_YY_actual,model,STATUS,scaler,min_max_scaler)
reshape_adv = np.reshape(np.array(adv),(len(adv)*WINDOW,len(header)))
df_adv = pd.DataFrame(reshape_adv,header)
