...
#####################data##################
#Read data
df_tr = pd.read_csv("normal_all.csv")
...
df_data_x = pd.read_csv("new_all_adv.csv")
...
adv = []
#len(df_x_scale)
for i in range(340001,len(data_x_win)):  
    print(i)
    sess.run(init)
    adv_i = sess.run(x_adv, feed_dict={model.input:data_x_win[i],y_true:np.array(df_YY_actual.iloc[i])})
    ...
    if i%10000 == 0:
        ...
        adv_temp.to_csv("defence_adv_backup1.csv",index = False)
...
df_adv.to_csv("defence_adv_all.csv",index = False)
print(df_TRUE.count(True)/len(df_TRUE))
